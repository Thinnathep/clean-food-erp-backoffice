  ...
  return (
    <div className="space-y-4 w-full text-center">
      <div className="flex items-center justify-between bg-white px-3 py-2 rounded-2xl border border-slate-100">
        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">โหมดสาย USB / COM:</span>
        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
          serialPort ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'
        }`}>
          <Power size={10} />
      ...
      <div className="flex justify-center w-full">
        {!serialPort ? (
          ...
          >
            {isConnectingSerial ? 'กำลังตรวจสอบพอร์ต...' : 'เชื่อมต่อผ่านสาย USB'}
          </button>
        ) : (
          <div className="flex gap-2 w-full max-w-[240px] justify-center">
            <button
              onClick={handleTestPrint}
              className="flex-1 py-1.5 bg-white hover:bg-slate-55 text-slate-700 rounded-xl text-xs font-bold transition-all border border-slate-200 active:scale-[0.98] flex items-center justify-center gap-1 cursor-pointer"
            >
              พิมพ์ทดสอบ
            </button>
            <button
