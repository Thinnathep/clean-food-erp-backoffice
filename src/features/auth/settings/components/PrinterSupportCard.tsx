    ...
    <div className="p-6 rounded-[2.5rem] bg-white border border-slate-100 shadow-sm flex flex-col justify-between space-y-4 w-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm transition-all ${
            printerEnabled ? 'bg-purple-50 text-purple-600' : 'bg-slate-100 text-slate-400'
          }`}>
            <Printer size={20} />
          </div>
          <div>
            <h4 className="text-sm font-bold text-slate-800">การใช้งานเครื่องพิมพ์</h4>
            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold">Printer Support</p>
          </div>
        </div>
        
        <button 
          onClick={() => updateSystemConfig({ printerEnabled: !printerEnabled })}
      ...
      {printerEnabled ? (
        ...
        <div className="py-8 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200 w-full">
          <p className="text-xs font-bold text-slate-400">ปิดการสั่งพิมพ์สำหรับหน้านี้แล้ว</p>
          <p className="text-[10px] text-slate-300">เปิดระบบสั่งพิมพ์ด้านบนเพื่อตั้งค่าฮาร์ดแวร์</p>
        </div>
      )}
    </div>
  );
