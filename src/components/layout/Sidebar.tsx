      ...
      `}>
        ...
        <div className={`p-4 md:p-6 flex items-center justify-between border-b border-slate-800 ${!isExpanded && 'md:justify-center'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
              ...
              <h1 className={`text-lg font-black tracking-tight leading-tight whitespace-nowrap transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 hidden md:block w-0'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1.5 scrollbar-hide">
          {navItems.map((item) => (
            <NavLink
              ...
              onClick={() => setMobileOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-xl transition-all font-bold text-sm whitespace-nowrap overflow-hidden group
        ...
        <div className="p-3 border-t border-slate-800 space-y-2">
          ...
          <div className={`px-2 py-2 mb-2 bg-slate-800/50 rounded-lg border border-slate-700/50 flex items-center gap-2 overflow-hidden ${!isExpanded && 'hidden md:flex justify-center'}`}>
             <div className="w-6 h-6 rounded-full bg-blue-500 flex-shrink-0 flex items-center justify-center text-[10px] font-bold">
          ...
          <button 
            title="Settings"
            className={`flex items-center gap-3 w-full rounded-xl transition-all font-bold text-sm text-slate-400 hover:text-white hover:bg-white/5
          ...
          <button 
            onClick={logout}
            title="Logout"
            className={`flex items-center gap-3 w-full rounded-xl transition-all font-bold text-sm text-red-400 hover:text-white hover:bg-red-500/20
              ${isExpanded ? 'px-4 py-3 text-left' : 'px-0 py-3 justify-center'}`}
          >
